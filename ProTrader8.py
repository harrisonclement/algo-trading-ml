...
import os
# Guarded import for mlflow so a missing package doesn't crash the app at import time
try:
    import mlflow
except Exception:
    mlflow = None
...
# NOTE: it's unsafe to hardcode tokens in source. Use Streamlit secrets or environment vars in production.
os.environ.setdefault("DATABRICKS_HOST", "https://dbc-b5e97478-b957.cloud.databricks.com/")
os.environ.setdefault("DATABRICKS_TOKEN", "dapi2806faac61e06eef37ff4a4f4a0d8f3b")
...
def load_predictor(run_id):
    ...
    if mlflow is None:
        # Fail gracefully and provide remediation steps
        st.error(
            "mlflow is not installed in this environment. "
            "Install it with `pip install mlflow` or add `mlflow` to your requirements.txt and redeploy."
        )
        return None
